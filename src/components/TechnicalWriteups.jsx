...
const TechnicalWriteups = () => {
  ...
  return (
    <section
      ...
      className="w-full noise-bg dark:bg-gray-900 py-20 px-4 md:px-8 lg:px-16"
      aria-label="Technical writeups and blog articles section">
      {/* Header */}
      <div className="mb-12 flex items-end justify-between">
        <div>
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium tracking-widest mb-2">
            {t.technicalWriteups?.newsroom || "BLOG"}
          </p>
          <h2 className="text-white dark:text-white text-5xl lg:text-6xl font-bold">
        ...
        <div className="flex gap-4 mb-2">
          <button
            onClick={() => scroll("left")}
            className="w-12 h-12 rounded-full bg-white dark:bg-gray-700 flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-300"
      ...
      {/* Error state */}
      {error && !loading && (
        <div className="bg-red-900 dark:bg-red-900 bg-opacity-20 dark:bg-opacity-40 border border-red-700 dark:border-red-600 rounded-lg p-6 text-red-600 dark:text-red-400">
        ...
        <>
          <div
            ...
            <div className="flex gap-6 pb-4">
              {articles.length === 0 ? (
                <div className="w-full flex items-center justify-center py-20 text-gray-400 dark:text-gray-500">
                  <p>
                ...
                articles.map((article) => (
                  <a
                    ...
                    <div className="h-full flex flex-col overflow-hidden transition-colors duration-300">
                      ...
                      {/* Content */}
                      <div className="flex-1 p-6 flex flex-col bg-white dark:bg-gray-800 rounded-b-2xl">
                        {/* Date */}
                        ...
                        {/* Title */}
                        <h3 className="text-gray-900 dark:text-white font-bold text-lg lg:text-xl leading-tight group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors duration-300 flex-1">
                          {article.title}
                        </h3>
                      </div>
                    </div>
      ...
      {!loading && !error && articles.length > 0 && (
        <div className="mt-8 flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 text-sm">
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
