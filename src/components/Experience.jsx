  ...
  return (
    <section className="relative w-full py-20 overflow-hidden">
      ...
      <div className="text-center mb-12">
        <h2 className="inline-flex items-baseline gap-3 text-[56px] font-light tracking-[-0.015em] text-gray-300">
      ...
      {/* Popup */}
      {activeItem && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 w-[380px] rounded-2xl bg-white/90 backdrop-blur-xl shadow-2xl p-6 animate-fade-in">
          <p className="text-xs uppercase tracking-wide text-neutral-500 mb-2">
            {activeItem.category}
          </p>

          <h3 className="text-xl font-bold">{activeItem.name}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {activeItem.skills.map((skill) => (
              <span
                key={skill}
                className="text-xs px-3 py-1 rounded-full bg-neutral-100"
              >
                {skill}
              </span>
            ))}
  ...
  return (
    ...
    >
      {item.logo && !imgError ? (
        ...
        <div className="text-sm font-medium text-center px-4">{item.name}</div>
      )}
    </div>
  )
}
